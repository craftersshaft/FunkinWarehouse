...
class PlayState extends MusicBeatState
{
	...
	public static var wareScore:Int = -1;
	
	public var curRandSong:Int = 1;
	public var warehouseSongs:Int = 5;	
	public var moneytext:Alphabet;
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
...

			{
				...
				case 4:
				if (curSong.startsWith('Warehouse')) {
				camhudVideo("assets/videos/warehouseintro.webm");
				}
			}
	...
	private function generateSong(dataPath:String):Void
	{
		...
		trace('will it break before me');
		warehouseSongs = songData.waresongs.length;
	...
	public function resetDadPos():Void {
	gf.visible = true;
	...
	public function playInst(audioPath:String, volume:Float = 1, looped:Bool = false):Void
	{
	trace(Paths.inst(audioPath));
	FlxG.sound.playMusic(Paths.inst(audioPath), volume, looped);
    }
	
	public function replaceNoteDataFromJSON(data:String):Void
	{
                    if (data != '') {
                    var parsedData:Array<SwagSection> = haxe.Json.parse(Assets.getText(Paths.json(data)));
					trace(parsedData[1]);
                    SONG.notes = parsedData;
					regenerateSong();
                    };
    }
	
	public function makeText(text:String,toBeCalled:String,yPos:Float = 0.0):Void
	{
	wareText = text;
	moneytext = new Alphabet(0, 0, wareText, true, false);
	moneytext.cameras = [PlayState.instance.camHUD];
	moneytext.screenCenter(X);
	if (yPos != 0.0){
	moneytext.y == yPos;
	} else {
	moneytext.screenCenter(Y);
	};
	#if windows
	ModchartState.luaSprites.set(toBeCalled,moneytext);
	#end
	PlayState.instance.addObject(moneytext);
	}
		...
		public function regenerateSong():Void
	{
		...
		trace("test" + unspawnNotes.length);
		// playerCounter += 1;
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (daNote.mustPress && !daNote.modifiedByLua)
					{
						...
						daNote.x = playerStrums.members[Math.floor(Math.abs(daNote.noteData))].x + 5;
						if (!daNote.isSustainNote)
					...
					else if (!daNote.wasGoodHit && !daNote.modifiedByLua)
					{
						...
						daNote.x = strumLineNotes.members[Math.floor(Math.abs(daNote.noteData))].x + 5;
						if (!daNote.isSustainNote)
	...
	public function NearlyEquals(value1:Float, value2:Float, unimportantDifference:Float = 10):Bool
		...
		private function keyShit():Void // I've invested in emma stocks
			...
			public function camhudVideo(source:String) // for background videos
				{
					#if cpp
					useVideo = true;
					...
					videoSprite.setGraphicSize(Std.int(videoSprite.width * 1));
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curSong.startsWith('Warehouse') && curBeat >= (SONG.notes.length * 4)) {
		var randomizer = new FlxRandom();
		var tempRandSong = FlxG.random.int(1,warehouseSongs);
		var nextRandSong:Int = tempRandSong;
		if (nextRandSong == curRandSong)
		{
		nextRandSong += 1;
		if (nextRandSong > warehouseSongs)
		{
		nextRandSong = 1;
		}
		}
		curRandSong = nextRandSong;
		replaceNoteDataFromJSON(curSong.toLowerCase() + "/" + nextRandSong);
		SONG.player2 = SONG.warechars[nextRandSong-1];
		playInst(curSong.toLowerCase() + "/" + nextRandSong);
		makeText(SONG.waresongs[nextRandSong-1], "randotext");
		remove(videoSprite);
		resetDadPos();
		wareScore += 1;
		
		}
		if (curSong.startsWith('Warehouse') && curBeat == 4) {
		PlayState.instance.removeObject(moneytext);
		}		
